state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "training", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("training")
...
start
{
    ...
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("training") &&
        !vars.hasStarted)
    {
