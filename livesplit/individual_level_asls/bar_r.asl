state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "bar_r", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("bar_r")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on bar_r level");
...
start
{
    ...
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("bar_r") &&
        !vars.hasStarted)
    {
