state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "mbase", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("mbase")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on mbase level");
...
start
{
    // Start when: game state becomes 1 AND map is "mbase"
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("mbase") &&
        !vars.hasStarted)
    {
