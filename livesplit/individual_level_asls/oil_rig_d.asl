state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "oil_rig_d", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("oil_rig_d")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on oil_rig_d level");
...
start
{
    // Start when: game state becomes 1 AND map is "oil_rig_d"
