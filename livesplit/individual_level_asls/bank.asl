state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "bank", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("bank")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on bank level");
...
start
{
    // Start when: game state becomes 1 AND map is "bank"
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("bank") &&
        !vars.hasStarted)
    {
