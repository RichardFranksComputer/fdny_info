state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "subway_a", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("subway_a")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on subway_a level");
...
start
{
    ...
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("subway_a") &&
        !vars.hasStarted)
    {
