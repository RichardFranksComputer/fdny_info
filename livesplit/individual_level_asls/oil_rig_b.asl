state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "oil_rig_b", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("oil_rig_b")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on oil_rig_b level");
...
start
{
    // Start when: game state becomes 1 AND map is "oil_rig_b"
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("oil_rig_b") &&
        !vars.hasStarted)
    {
