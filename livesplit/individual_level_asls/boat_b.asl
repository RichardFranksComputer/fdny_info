state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "boat_b", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("boat_b")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on boat_b level");
...
start
{
    // Start when: game state becomes 1 AND map is "boat_b"
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("boat_b") &&
        !vars.hasStarted)
    {
