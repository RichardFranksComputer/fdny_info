state("FDNYFirefighter", "1.0")
{
     ...
     *   Values: Level names like "boat_a", "mission1", etc.
     *   Usage Examples:
     *     - Check specific level: current.mapName.ToLower().Contains("boat_a")
...
startup
{
    settings.Add("autostart", true, "Auto-start timer on boat_a level");
...
start
{
    // Start when: game state becomes 1 AND map is "boat_a"
    ...
    if (settings["autostart"] && 
        current.gameState == 1 && 
        old.gameState != 1 && 
        current.mapName.ToLower().Trim().Contains("boat_a") &&
        !vars.hasStarted)
    {
